 ...
 *
 */
public class Int extends BaseFunction {
	
	public int process(double number) {
		return (int)Precision.round(number, 0);
	}

	public Object process(String number) {
		if(StringUtils.isNumeric(number)) {
			return (int) Precision.round(Double.parseDouble(number), 0);
		} else {
			return number;
		}
	}
