...
public abstract class BaseElement implements Element {
	...
	public int invoke() throws UnsupportedScriptException {
		...
		LinkedHashMap<String, Object> parsedAttributes = new LinkedHashMap();
		for(Method method: methods) {
					...
					try {
						...
						if(parsed) {
							if(reparse) {
								...
								if(parsedAttributes.containsKey(attributeName)) attrValue = parsedAttributes.get(attributeName);
								else {
									attrValue = env.parse(attributes.get(attributeName));
									parsedAttributes.put(attributeName, attrValue);
								}
							}
						} else {
